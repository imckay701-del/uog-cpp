...
print(f"X_poly =\n{X_poly[:4]}") # print the first 3 samples
...
## The barely minimum you must do {#sec-bare-minimum-task}

1. Decide your own OOP design to implement the library. **Note** that a functioning program is not necessary a well-designed programe. You need to make your own decision.
...
3. Add these two classes into the namespace `linear_model`. This `linear_model` is again a namespace inside a bigger namespace `sklearn_cpp`.
...
4. Write a main program to apply these two classes to solve two problems:
...
## The not-barely minimum you must do
